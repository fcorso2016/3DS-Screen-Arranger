...
(function () {
    ...
    csInterface.evalScript('init()', null);
}());
...
function saveFile() {
    ...
    if(docCounter > 0) {
        csInterface.evalScript('saveJSXFile('+JSON.stringify(document.getElementsByClassName("scriptFileTab")[0].innerHTML)+', '+JSON.stringify((<HTMLInputElement>document.getElementsByClassName("scriptFileTextArea")[0]).value)+')', function() {
    ...
    if(docCounter > 0) {
        csInterface.evalScript('saveJSXFile('+JSON.stringify(document.getElementsByClassName("scriptFileTab")[0].innerHTML)+', '+JSON.stringify((<HTMLInputElement>document.getElementsByClassName("scriptFileTextArea")[0]).value)+')', function() {
            csInterface.evalScript("chooseJSXFile()", function(res) {
                ...
                (<HTMLInputElement>document.getElementsByClassName("scriptFileTextArea")[0]).value = JSON.parse(res).code;
            });
        });
    } else {
...
function playClick() {
    var csInterface = new CSInterface();
    if(docCounter > 0) {
        csInterface.evalScript('runScriptFile('+JSON.stringify(document.getElementsByClassName("scriptFileTab")[0].innerHTML)+')', null);
    }
}
        ...
        if(JSON.parse(res) != null) {
            if(docCounter == 1) {
                csInterface.evalScript('saveJSXFile('+JSON.stringify(document.getElementsByClassName("scriptFileTab")[0].innerHTML)+', '+JSON.stringify((<HTMLInputElement>document.getElementsByClassName("scriptFileTextArea")[0]).value)+')', function() {
                    ...
                    var scriptFileTextArea = <HTMLInputElement>document.getElementsByClassName("scriptFileTextArea")[0];
